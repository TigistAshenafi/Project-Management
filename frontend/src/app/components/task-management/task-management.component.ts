...
export class TaskManagementComponent implements OnInit {
  ...
  minDate: string = '';
  maxDate: string = '';
  remainingHours: { [taskId: number]: number } = {};
...
addTasks(): void {
  ...
  loadTask(): void {
    this.taskService.getAllTasks().subscribe({
      next: (data) => {
        this.tasks = data;
        this.fetchRemainingHoursForTasks();
      },
      error: (err) => {
  ...
  fetchRemainingHoursForTasks() {
    const userId = this.authService.getCurrentUserId();
    if (!userId) return;
    this.tasks.forEach(task => {
      this.taskService.getRemainingHours(userId, task.id).subscribe({
        next: (res) => {
          this.remainingHours[task.id] = res.remaining_hours;
        },
        error: (err) => {
          this.remainingHours[task.id] = -1; // Indicate error
        }
      });
    });
  }
